...
def compute_gaussian_proab(x, mean, var):
    res = 1
...
def main():
    ...
    #seperate the last column
    df2 = dfrandom.iloc[:,4]
    #concat the 4 numerical and the last column
    ...
    for i in range(0,len(dftest)):
        ...
        probC1 = compute_gaussian_proab(x,mean_setosa.values,var_setosa.values)
        probC2 = compute_gaussian_proab(x,mean_versicolor.values,var_versicolor.values)
        probC3 = compute_gaussian_proab(x,mean_virginica.values,var_virginica.values)
